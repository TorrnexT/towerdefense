...
- `.monitoring/vhosts.yaml`: Ein gemeinsamer HTTP-VHost ohne Domain für `http://SERVER-IP/towerdefense/` (Frontend) und `/towerdefense/api/` (Backend), einschließlich WebSocket-Upgrades. `/` und `/towerdefense` werden auf `/towerdefense/` umgeleitet. Der Client-Workflow setzt `VITE_BASE_PATH=/towerdefense/` und `VITE_SERVER_URL=/towerdefense/api`; Assets, Manifest und Offline-Paket berücksichtigen den Unterpfad. Der Healthcheck liegt unter `/towerdefense/api/health`. Der Proxy entfernt den API-Präfix vor der Weiterleitung an Port 2567. Über HTTP mit Server-IP läuft das Spiel im Gastmodus: Fortschritt bleibt nur bis zum Neuladen im Arbeitsspeicher. Verschlüsselte dauerhafte Spielstände und Offline-Caching bleiben HTTPS bzw. localhost vorbehalten. Änderungen müssen ins konfigurierte Git-Repository übertragen und danach über Monitoring-Refresh und einen neuen Build derselben Version übernommen werden.
...
### HTTP-Unterpfad prüfen

`VITE_BASE_PATH=/towerdefense/ VITE_SERVER_URL=/towerdefense/api pnpm build` erzeugt denselben Client wie der Monitoring-Workflow. Danach prüft `pnpm verify:subpath` mit lokalem Nginx und Chrome die Weiterleitung, HTTP-Gastmodus, Einzelspieler, Koop-WebSockets sowie den Offline-Reload unter localhost. Falls nötig, `NGINX_BINARY` und `CHROME_PATH` auf die installierten Programme setzen. Der Test nutzt freie Ports und räumt seine temporären Dateien und Prozesse auf.
